...
WITH d AS (SELECT taxon_key, dataset_key, count(*) AS n, max(year) AS y1 FROM {{src}} WHERE taxon_key IS NOT NULL GROUP BY 1, 2)
SELECT d.taxon_key, any_value(t.scientific_name) AS scientific_name, any_value(t.common_name) AS common_name,
       ...
       sum(d.n)::BIGINT AS n, max(d.y1) AS y1, string_agg(d.dataset_key, ',' ORDER BY d.n DESC) AS datasets

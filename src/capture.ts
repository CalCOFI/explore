...
import { fontEmbedCss, DEFAULT_THEME } from "./brand";
...
export async function captureView(o: CaptureOpts): Promise<HTMLCanvasElement> {
  ...
  const bg = getComputedStyle(document.documentElement).getPropertyValue("--bg").trim() || (DEFAULT_THEME === "dark" ? "#1b1d20" : "#ffffff");
  // brand v2 sets the page in Source Sans 3 (self-hosted, CORS *): hand html-to-image the @font-face rules with the woff2
  // inlined, or the clone falls back to the system stack. v1 has no webfonts and gets "" (skipFonts stays: theme.css is
  // cross-origin and cssRules throws on it)
  ...
  const shot = await toCanvas(root, { pixelRatio: scale, backgroundColor: bg, filter: (n) => !skip(n as HTMLElement), cacheBust: false, skipFonts: true, fontEmbedCSS: fontEmbedCSS || undefined, style: rootStyle });

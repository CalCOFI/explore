...
export interface Sel {
  ...
  baseLabels: boolean;         // `basemap=nolabels`: CARTO's own text (place names, road names, POIs, water names) off — a data-centric view (Ben, 2026-09-09)
  basemapo: number | null;     // CARTO's text opacity 0.1–1 (`basemapo=0.4`); null = the theme default (0.6 dark · 0.7 light) — the labels are restyled quieter either way
...
export function fromUrl(): Sel {
  ...
  return {
    ...
    land: p.get("land") !== "off",
    baseLabels: p.get("basemap") !== "nolabels",
    basemapo: (v => v != null && v !== "" && isFinite(+v) && +v >= 0.1 && +v <= 1 ? Math.round(+v * 100) / 100 : null)(p.get("basemapo")),
...
export function toUrl(s: Sel) {
  ...
  if (!s.baseLabels) p.set("basemap", "nolabels");
  if (s.basemapo != null) p.set("basemapo", s.basemapo.toFixed(2));

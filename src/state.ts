...
export interface Sel {
  ...
  labels: boolean;             // contour lens: the level labels along the isolines (`labels=on`; off by default until they are legible — D44)
  realm: Realm;
...
export const DEFAULTS: Sel = {
  lens: "station", res: 5, interp: "ok", surface: "value", grain: "site", inputs: null, labels: false, realm: "bio", taxon: DEFAULT_TAXON, var: "temperature",
...
export function toUrl(s: Sel) {
  ...
  if (s.lens === "contour") { if (s.interp !== DEFAULTS.interp) p.set("interp", s.interp); if (s.surface !== DEFAULTS.surface) p.set("surface", s.surface); if (s.grain !== DEFAULTS.grain) p.set("grain", s.grain); if (s.inputs != null) p.set("inputs", s.inputs ? "on" : "off"); if (s.labels) p.set("labels", "on"); }
